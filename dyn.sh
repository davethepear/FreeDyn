...
new=$(wget -qO- $ipfile)
if [ -z "new" ]; then echo "Asking icanhazip for your IP" | new=$(wget -q -O - http://icanhazip.com/) ; fi
if [ -z "new" ]; then echo "Asking google for your IP" | new=$(dig TXT +short o-o.myaddr.l.google.com @ns1.google.com | awk -F'"' '{ print $2}') ; fi
if [ -z "new" ]; then echo "Asking opendns for your IP" | new=$(dig +short myip.opendns.com @resolver1.opendns.com) ; fi
if [ -z "new" ]; then echo "Unable to get current IP, possibly offline" | exit 1 ; fi
